<?php 
	...
	if($_SERVER['REQUEST_METHOD']=='POST'){
		...
		$kec = "SELECT * FROM DESA JOIN KECAMATAN ON DESA.ID_KECAMATAN=KECAMATAN.ID_KECAMATAN WHERE DESA.ID_KECAMATAN='$id'";
		...
		while($row1 = mysqli_fetch_array($r1)){
			array_push($a_desa,array(
				"id"=>$row1['ID_DESA'],
				"ket"=>$row1['KET_DESA'],
				"batas_wilayah"=>$row1['BATAS_WILAYAH_DESA'],
				"koordinat"=>$row1['KOORDINAT_DESA'],
				"koordinat2"=>$row1['KOORDINAT_KECAMATAN'],
		...
		foreach ($a_desa as $row1){
			...
			WHERE DATA_KEPENDUDUKAN_DESA.ID_DATA_DESA LIKE '%$id' ORDER BY TAHUN.KET_TAHUN ASC";
			$r = mysqli_query($con,$sql);
			$data=false;
			$no=1;
			while ($row = mysqli_fetch_array($r)) {
				if ($no==mysqli_num_rows($r)) {
					...
					"data"=>"true",
					"id"=>$row['ID_DESA'],
					"ket"=>$row['KET_DESA'].",".$row['LUAS_WILAYAH_DESA'].",".$row['KET_KECAMATAN'],
					"batas_wilayah"=>$row['BATAS_WILAYAH_DESA'],
					"koordinat"=>$row['KOORDINAT_DESA'],
					"koordinat2"=>$row['KOORDINAT_KECAMATAN'],
					"data_detail"=>$detail,
					));
					$data=true;
				}
			...
			if (!$data) {
				...
				"koordinat2"=>$array_data['koordinat2'],
				"data_detail"=>"",
				));
			}
			$i++;
